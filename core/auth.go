...
type Authorizer interface {
	...
	GrantRoot(ctx context.Context, tx *sql.Tx, witness *v0.Witness) (bool, error)

	// GrantBackupRoot will give root access if primary/backup keys are sufficient
	GrantBackupRoot(ctx context.Context, tx *sql.Tx, witness *v0.Witness) (bool, error)
	...
	HaveOwners(ctx context.Context, tx *sql.Tx) (bool, error)

	// GetOwners returns the current owners, or an error if no owners are set or if
	// there is some IO error
	GetOwners(ctx context.Context, tx *sql.Tx) (*v0.OwnerSet, error)

	SetOwners(ctx context.Context, tx *sql.Tx, set *v0.OwnerSet) error
}
